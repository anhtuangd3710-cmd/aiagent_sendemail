...
class ChatbotService:
    ...
    # Database schema for AI to understand and generate queries
    DATABASE_SCHEMA = """
=== DATABASE SCHEMA ===
...
1. BẢNG users (Người dùng):
   ...
   - role: VARCHAR(50) - Vai trò ('admin' hoặc 'user')
   - is_active: BOOLEAN - Đang hoạt động
   - created_at: TIMESTAMP - Ngày tạo
   - last_login: TIMESTAMP - Lần đăng nhập cuối

2. BẢNG sent_emails (Email đã gửi):
   - id: SERIAL PRIMARY KEY
   - user_id: INTEGER REFERENCES users(id) - ID người gửi
   ...
   - body: TEXT - Nội dung email
   - purpose: TEXT - Mục đích gửi
   - sent_at: TIMESTAMP - Thời gian gửi
   - response_received: BOOLEAN - Đã nhận phản hồi chưa
   - email_type: VARCHAR(50) - Loại email ('general', 'cv_invitation', etc.)
   - cv_evaluation_id: INTEGER - Liên kết với đánh giá CV (nếu có)

3. BẢNG responses (Phản hồi email):
   - id: SERIAL PRIMARY KEY
   - sent_email_id: INTEGER REFERENCES sent_emails(id) - ID email gốc
   - response_subject: TEXT - Tiêu đề phản hồi
   - response_body: TEXT - Nội dung phản hồi
   - received_at: TIMESTAMP - Thời gian nhận
   - analysis: JSONB - Phân tích AI (sentiment, decision, etc.)
   - notification_sent: BOOLEAN - Đã gửi thông báo chưa

4. BẢNG cv_evaluations (Đánh giá CV):
   - id: SERIAL PRIMARY KEY
   - user_id: INTEGER REFERENCES users(id)
   - candidate_name: VARCHAR(255) - Tên ứng viên
   - candidate_email: VARCHAR(255) - Email ứng viên
   - job_title: VARCHAR(255) - Vị trí tuyển dụng
   - job_requirements: TEXT - Yêu cầu công việc
   - company_name: VARCHAR(255) - Tên công ty
   - cv_content: TEXT - Nội dung CV
   - overall_score: DECIMAL(5,2) - Điểm tổng (0-100)
   - is_qualified: BOOLEAN - Đạt yêu cầu (>=85%)
   - evaluation_result: JSONB - Kết quả đánh giá chi tiết
   - email_sent: BOOLEAN - Đã gửi email mời phỏng vấn
   - status: VARCHAR(50) - Trạng thái ('pending', 'completed', etc.)
   - created_at: TIMESTAMP

5. BẢNG chat_sessions (Phiên chat):
   - id: SERIAL PRIMARY KEY
   - user_id: INTEGER REFERENCES users(id)
...
6. BẢNG chat_messages (Tin nhắn chat):
   - id: SERIAL PRIMARY KEY
...
=== MỐI QUAN HỆ ===
- users (1) -> (N) sent_emails: Một user có thể gửi nhiều email
- sent_emails (1) -> (N) responses: Một email có thể có nhiều phản hồi
- users (1) -> (N) cv_evaluations: Một user có thể đánh giá nhiều CV
- cv_evaluations (1) -> (1) sent_emails: Một CV có thể liên kết với email mời phỏng vấn
- users (1) -> (N) chat_sessions: Một user có nhiều phiên chat
"""
    ...
    def execute_dynamic_query(self, sql: str, user_id: int, is_admin: bool = False) -> Dict:
        """Execute a dynamic SQL query generated by AI with safety checks"""
        try:
            # Safety: Only allow SELECT queries
            sql_upper = sql.strip().upper()
            if not sql_upper.startswith('SELECT'):
                return {'success': False, 'error': 'Chỉ cho phép câu lệnh SELECT', 'data': []}
            
            # Safety: Block dangerous keywords
            dangerous_keywords = ['DROP', 'DELETE', 'UPDATE', 'INSERT', 'ALTER', 'TRUNCATE', 'GRANT', 'REVOKE']
            for keyword in dangerous_keywords:
                if keyword in sql_upper:
                    return {'success': False, 'error': f'Không cho phép sử dụng {keyword}', 'data': []}
            
            # Safety: For non-admin, restrict to user's own data
            if not is_admin:
                # Inject user_id filter if querying user-specific tables
                user_tables = ['sent_emails', 'cv_evaluations', 'chat_sessions', 'chat_messages']
                for table in user_tables:
                    if table in sql.lower():
                        # Check if WHERE clause exists
                        if 'WHERE' in sql_upper:
                            # Add AND user_id condition if not already present
                            if 'user_id' not in sql.lower():
                                sql = sql.replace('WHERE', f'WHERE user_id = {user_id} AND', 1)
                        else:
                            # Find table name and add WHERE clause
                            # This is a simplified approach - in production, use proper SQL parsing
                            if 'FROM' in sql_upper:
                                parts = sql.split('FROM', 1)
                                after_from = parts[1].strip()
                                # Find table alias or name
                                table_part = after_from.split()[0]
                                if table_part.lower() in user_tables or table_part.lower() == table:
                                    if 'ORDER BY' in sql_upper:
                                        sql = sql.replace('ORDER BY', f'WHERE user_id = {user_id} ORDER BY', 1)
                                    elif 'GROUP BY' in sql_upper:
                                        sql = sql.replace('GROUP BY', f'WHERE user_id = {user_id} GROUP BY', 1)
                                    elif 'LIMIT' in sql_upper:
                                        sql = sql.replace('LIMIT', f'WHERE user_id = {user_id} LIMIT', 1)
                                    else:
                                        sql = sql.rstrip(';') + f' WHERE user_id = {user_id}'
            
            # Execute query
            result = self.database.query_raw(sql)
            ...
            max_rows = 100
            data = result[:max_rows] if len(result) > max_rows else result
    ...
    def _parse_sql_from_response(self, response: str) -> Optional[str]:
        ...
        # Pattern 2: SQL tag
        sql_tag = re.search(r'\[SQL:\s*(.*?)\]', response, re.DOTALL)
        if sql_tag:
            return sql_tag.group(1).strip()
        
        # Pattern 3: SQL after "Query:" or "SQL:"
        sql_label = re.search(r'(?:Query|SQL):\s*\n?(SELECT.*?)(?:\n\n|\[INTENT|$)', response, re.DOTALL | re.IGNORECASE)
        if sql_label:
            return sql_label.group(1).strip()
    ...
    def _format_query_result(self, result: Dict) -> str:
        ...
        if not result.get('success'):
            return f"❌ Lỗi truy vấn: {result.get('error', 'Unknown error')}"
        ...
        # Format as table or list based on data structure
        if len(data) <= 10:
            # Show all rows
            formatted = f"📊 Tìm thấy {row_count} kết quả:\n\n"
            ...
            if result.get('truncated'):
                formatted += f" (hiển thị {len(data)} dòng đầu tiên)"
            formatted += ":\n\n"
            ...
            for i, row in enumerate(data[:5], 1):
                formatted += f"**{i}.** "
                items = []
                for key, value in list(row.items())[:5]:  # Limit columns too
                    if value is not None:
                        if isinstance(value, datetime):
                            value = value.strftime('%d/%m/%Y %H:%M')
...
"""
    ...
    def process_query(self, user_id: int, query: str, user_settings: Dict = None, 
        ...
        """Process user query and return appropriate response with data using dynamic SQL"""
        try:
            ...
            if is_admin and user_stats:
                admin_context = f"""
=== QUYỀN ADMIN ===
Bạn đang có quyền ADMIN - có thể truy vấn DỮ LIỆU TOÀN HỆ THỐNG.
- Tổng số người dùng: {user_stats.get('total_users', 0)}
- Tổng email toàn hệ thống: {user_stats.get('total_emails_all', 0)}
- Tổng CV đã đánh giá: {user_stats.get('total_cv_all', 0)}
"""
            ...
            view_note = "(Admin - Toàn hệ thống)" if is_admin else f"(user_id = {user_id})"
            
            # Build the enhanced prompt with SQL generation capability
            context = f"""
...
"""
            ...
            # Clean the response (remove intent line and SQL blocks)
            clean_response = self._clean_response(ai_response)
            
            # Extract and execute SQL if present
            sql_query = self._parse_sql_from_response(ai_response)
            sql_result_text = ""
            if sql_query:
                logger.info(f"Dynamic SQL detected: {sql_query[:100]}...")
                sql_result = self.execute_dynamic_query(sql_query, user_id, is_admin)
                if sql_result['success']:
                    sql_result_text = self._format_query_result(sql_result)
                    clean_response = clean_response.rstrip() + "\n\n" + sql_result_text
                else:
                    clean_response = clean_response.rstrip() + f"\n\n⚠️ Không thể thực hiện truy vấn: {sql_result['error']}"
